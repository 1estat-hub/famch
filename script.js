...
// Отслеживание изменения статусов сообщений
function watchMessageStatusChanges(currentRoom, currentUserDisplayName) {
  // Слушаем изменения ТОЛЬКО своих сообщений
  db.ref('messages/' + currentRoom).orderByChild('name').equalTo(currentUserDisplayName)
    .on('child_changed', snapshot => {
      ...
      if (!msgDiv || !msgDiv.classList.contains('me')) return;
...
function markMessagesAsRead(currentRoom, currentUserDisplayName) {
  ...
  db.ref('messages/' + currentRoom).once('value', snapshot => {
    ...
    snapshot.forEach(msgSnap => {
      ...
      // Помечаем как прочитанные только:
      // 1. Сообщения от другого пользователя
      // 2. Сообщения со статусом 'delivered' (или без статуса)
      // 3. Сообщения, которые были отправлены ДО входа в чат
      if (msg.name !== currentUserDisplayName && 
          msg.status !== 'read' && 
          msg.time < timeWhenEntered) {
        updates[key] = { status: 'read' };
      }
    });
    
    // Если есть сообщения для пометки как прочитанные
